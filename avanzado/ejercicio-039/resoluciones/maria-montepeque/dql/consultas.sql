...
FROM inventario_kickboxing_avanzado
ORDER BY stock ASC;

-- 2. Equipo por debajo o en su stock minimo (necesita reabastecimiento)
...
WHERE OBJECT_NAME = 'inventario_kickboxing_avanzado';
...
-- 5. Equipo con mayor stock actual
SELECT nombre, stock
FROM inventario_kickboxing_avanzado
ORDER BY stock DESC
LIMIT 3;

-- 6. Total de unidades en inventario, agrupado por unidad de medida
...
-- se abrieron DOS conexiones simultaneas contra 'Guantes de kickboxing
-- 16oz' (id_equipo = 2):
--   Sesion A: START TRANSACTION; SELECT ... FOR UPDATE; SELECT SLEEP(4); UPDATE ...; COMMIT;
--   Sesion B (1 segundo despues): UPDATE inventario_kickboxing_avanzado SET stock = stock - 1 WHERE id_equipo = 2;
-- Resultado real: la sesion B quedo esperando y su UPDATE solo se
-- completo unos 3 segundos despues (justo cuando la sesion A hizo
-- COMMIT), confirmando que FOR UPDATE bloquea la fila para otras
