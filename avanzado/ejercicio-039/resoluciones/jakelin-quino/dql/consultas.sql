...
SELECT * FROM `ejercicio-039-adv-luchadores`;

-- 2. Ver bitácora vacía
SELECT * FROM `ejercicio-039-adv_bitacora`;
...
FROM `ejercicio-039-adv-luchadores`
ORDER BY id;
...
LOCK TABLES `ejercicio-039-adv-luchadores` READ;
SELECT * FROM `ejercicio-039-adv-luchadores`;
UNLOCK TABLES;

-- 8. Simular bloqueo de escritura
LOCK TABLES `ejercicio-039-adv-luchadores` WRITE;
UPDATE `ejercicio-039-adv-luchadores` 
