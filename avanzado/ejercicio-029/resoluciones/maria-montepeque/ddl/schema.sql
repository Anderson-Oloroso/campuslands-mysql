...
DROP EVENT IF EXISTS ev_expirar_ofertas_avanzado;
DROP TABLE IF EXISTS auditoria_ofertas_avanzado;
DROP TABLE IF EXISTS ofertas_avanzado;

CREATE TABLE ofertas_avanzado (
  ...
  CONSTRAINT chk_ofertas_avanzado_precios CHECK (precio_oferta < precio_original)
) ENGINE=InnoDB;

CREATE TABLE auditoria_ofertas_avanzado (
...
CREATE EVENT ev_expirar_ofertas_avanzado
ON SCHEDULE EVERY 1 HOUR
STARTS CURRENT_TIMESTAMP
DO
BEGIN
  INSERT INTO auditoria_ofertas_avanzado (id_oferta, accion)
