...
(`ofertas_avanzado`, `auditoria_ofertas_avanzado`) y cree un
**Event Scheduler** (`ev_expirar_ofertas_avanzado`) que revisa cada
hora si alguna oferta ya vencio (`fecha_fin < NOW()`) y la marca como
`'expirada'`, dejando rastro en la auditoria. Nadie tiene que acordarse
de correr ese `UPDATE` a mano.
...
1. Antes: 3 ofertas (`Collar de Perlas`, `Pulsera de Plata`,
   `Collar Minimalista`) ya tenian `fecha_fin < NOW()` pero seguian en
...
3. Despues: las 3 ofertas aparecieron en estado `'expirada'`, y la
   tabla `auditoria_ofertas_avanzado` tenia exactamente 3 filas
   nuevas, una por cada oferta, con la hora real de ejecucion.
4. El evento de prueba (`ON SCHEDULE AT`, de una sola vez) desaparecio
   solo de `SHOW EVENTS` al terminar — el comportamiento por defecto
   de MySQL (`ON COMPLETION NOT PRESERVE`) para eventos `ONE TIME`.

El evento real de este ejercicio (`ev_expirar_ofertas_avanzado`) es
`RECURRING` (`EVERY 1 HOUR`) y sigue activo despues de correr
`dml/inserts.sql`; la consulta 6 de `dql/consultas.sql` lo confirma
consultando `information_schema.EVENTS`.
