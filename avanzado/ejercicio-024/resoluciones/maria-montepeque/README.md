...
## Que hice
Modele el inventario de materiales de un taller de soldadura
(`inventario_soldadura_avanzado`) y practique dos mecanismos de
...
simultaneas de verdad contra la misma fila (`Electrodo E7018`,
`id_material = 2`):
...
Resultado real observado: la sesion B quedo esperando y su `UPDATE`
solo se completo unos **3 segundos despues**, justo cuando la sesion A
hizo `COMMIT`. Esto confirma que `FOR UPDATE` bloquea la fila para
otras conexiones hasta que la transaccion termina, no es solo
...
Lo confirme al ejecutar el script:

```
ERROR 1142 (42000): SELECT command denied to user 'campus'@'...' for table 'data_locks'
