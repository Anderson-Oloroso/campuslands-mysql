...
FROM `ejercicio-022-adv-proyectos`
GROUP BY estudio;
...
FROM `ejercicio-022-adv-personajes` pe
INNER JOIN `ejercicio-022-adv-proyectos` p ON pe.proyecto_id = p.id;
...
FROM `ejercicio-022-adv-proyectos` p
LEFT JOIN `ejercicio-022-adv-personajes` pe ON p.id = pe.proyecto_id
GROUP BY p.id;
