...
## Que hice
Modele la plantilla de jugadores de un equipo de futbol sala
(`jugadores_roles_avanzado`) y cree tres roles de MySQL con distinto
nivel de acceso, aplicando el principio de menor privilegio:

| Rol | Privilegios sobre la tabla |
| --- | --- |
| `rol_futsal_lector` | `SELECT` |
| `rol_futsal_editor` | `SELECT`, `INSERT`, `UPDATE` |
| `rol_futsal_admin` | Todos (`ALL PRIVILEGES`) |

Cada rol se asigno a un usuario de ejemplo (`laura_lectora`,
`mario_editor`, `valeria_admin`) con `SET DEFAULT ROLE`, para que el
rol quede activo automaticamente al iniciar sesion.
...
```bash
mysql -u laura_lectora -p'Lectora#2026' -e "SELECT COUNT(*) FROM campuslands_mysql.jugadores_roles_avanzado;"
# -> funciona, devuelve 8
...
# -> ERROR 1142 (42000): INSERT command denied to user 'laura_lectora'@'...'
#    for table 'jugadores_roles_avanzado'
```
...
- Los `GRANT` son sobre la tabla especifica
  (`campuslands_mysql.jugadores_roles_avanzado`), no sobre todo el
