...
-- 1. RANK de jugadores por daño total infligido (todos los enfrentamientos)
WITH danio_total AS (
  SELECT jugador, SUM(danio_infligido) AS danio_total
  FROM enfrentamientos_avanzado
  GROUP BY jugador
)
...
-- 5. Cuartiles de enfrentamientos segun el daño infligido (NTILE)
