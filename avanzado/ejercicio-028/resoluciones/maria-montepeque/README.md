...
## Que hice
Modele perfiles de estudiantes de una academia tech en
`estudiantes_avanzado`, guardando en una columna `JSON` los datos
...
- Los `INSERT` arman el JSON con `JSON_OBJECT`/`JSON_ARRAY` en vez de
  ...
  al construirlo.
- Andres Castro empieza con `"proyectos": []` (arreglo vacio) a
...
## Consultas incluidas
1. Extraer el usuario de GitHub de cada estudiante (`->>`).
2. Estudiantes con "Python" entre sus habilidades (`JSON_CONTAINS`).
3. Habilidades expandidas en filas (`JSON_TABLE`).
4. Cantidad de habilidades y proyectos por estudiante (`JSON_LENGTH`).
5. Estudiantes con mas de un proyecto registrado.
6. Confirmacion de que Andres Castro ya tiene su proyecto (`JSON_ARRAY_APPEND`).
