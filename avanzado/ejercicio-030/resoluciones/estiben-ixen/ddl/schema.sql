...
-- Tema: equipo de streaming (optimizacion)
-- Ejecutar este archivo antes de inserts.sql

CREATE DATABASE IF NOT EXISTS campuslands_mysql;
USE campuslands_mysql;

DROP TABLE IF EXISTS log_auditoria_streaming;
...
CREATE INDEX idx_streaming_fk ON dispositivos_setup(id_streamer);
CREATE INDEX idx_streaming_metric ON dispositivos_setup(marca DESC);
CREATE INDEX idx_streaming_compuesto ON dispositivos_setup(id_streamer, marca);
