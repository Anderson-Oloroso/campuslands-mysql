-- Datos de practica: 5000 eventos de chat sinteticos, generados con un
-- CTE recursivo, repartidos entre 5 streamers. Se necesita este
...
INSERT INTO eventos_chat_avanzado (streamer, usuario, tipo_evento, fecha)
...
SELECT
  ELT(1 + ((n + m.mult) % 5), 'NightOwlGaming', 'PixelQueenArt', 'ChillBeatsRadio', 'CodeWithMaria', 'RetroArcadeLive') AS streamer,
  CONCAT('user_', n, '_', m.mult) AS usuario,
  ELT(1 + ((n * m.mult) % 4), 'mensaje', 'suscripcion', 'donacion', 'ban') AS tipo_evento,
