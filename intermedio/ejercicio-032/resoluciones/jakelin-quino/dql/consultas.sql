...
FROM `ejercicio-032-int-jugadores` j
LEFT JOIN `ejercicio-032-int-equipos` e ON j.equipo = e.id;

-- 2. Equipos con cantidad de jugadores
SELECT e.nombre, COUNT(j.id) AS total
FROM `ejercicio-032-int-equipos` e
LEFT JOIN `ejercicio-032-int-jugadores` j ON e.id = j.equipo
GROUP BY e.id, e.nombre;

-- 3. Jugadores sin equipo (NULL)
SELECT nick, puntaje FROM `ejercicio-032-int-jugadores` 
WHERE equipo IS NULL;

-- 4. Equipos sin jugadores
SELECT e.nombre FROM `ejercicio-032-int-equipos` e
LEFT JOIN `ejercicio-032-int-jugadores` j ON e.id = j.equipo
WHERE j.id IS NULL;
