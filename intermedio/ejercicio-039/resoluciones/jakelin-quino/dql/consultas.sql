...
SELECT * FROM `ejercicio-039-int-categorias`;

-- 2. Ver todos los gimnasios
SELECT * FROM `ejercicio-039-int-gimnasios`;

-- 3. Luchadores con su categoría y gimnasio
SELECT l.nombre, c.nombre AS categoria, g.nombre AS gimnasio, l.victorias
FROM `ejercicio-039-int-luchadores` l
INNER JOIN `ejercicio-039-int-categorias` c ON l.categoria_id = c.id
INNER JOIN `ejercicio-039-int-gimnasios` g ON l.gimnasio_id = g.id;

-- 4. Luchadores por categoría
SELECT c.nombre AS categoria, COUNT(l.id) AS total_luchadores
FROM `ejercicio-039-int-categorias` c
LEFT JOIN `ejercicio-039-int-luchadores` l ON c.id = l.categoria_id
GROUP BY c.id;

-- 5. Luchadores de gimnasios específicos
SELECT g.nombre AS gimnasio, l.nombre AS luchador, l.victorias
FROM `ejercicio-039-int-gimnasios` g
INNER JOIN `ejercicio-039-int-luchadores` l ON g.id = l.gimnasio_id
