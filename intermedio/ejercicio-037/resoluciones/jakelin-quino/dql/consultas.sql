...
SELECT * FROM `ejercicio-037-int-equipos`;

-- 2. Estadísticas con equipo y temporada
SELECT e.nombre AS equipo, t.año, es.puntos, es.ganados, es.goles_favor
FROM `ejercicio-037-int-estadisticas` es
INNER JOIN `ejercicio-037-int-equipos` e ON es.equipo_id = e.id
INNER JOIN `ejercicio-037-int-temporadas` t ON es.temporada_id = t.id;
