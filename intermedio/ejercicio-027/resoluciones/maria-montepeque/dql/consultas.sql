...
FROM sesiones_juego_intermedio
GROUP BY jugador
...
SELECT MONTHNAME(fecha_sesion) AS mes, SUM(duracion_min) AS minutos_jugados
FROM sesiones_juego_intermedio
GROUP BY MONTH(fecha_sesion), MONTHNAME(fecha_sesion)
ORDER BY MONTH(fecha_sesion);

-- 5. Porcentaje de sesiones sin ninguna muerte, sobre el total de sesiones
SELECT
  COUNT(*) AS total_sesiones,
  SUM(CASE WHEN muertes = 0 THEN 1 ELSE 0 END) AS sesiones_sin_muertes,
