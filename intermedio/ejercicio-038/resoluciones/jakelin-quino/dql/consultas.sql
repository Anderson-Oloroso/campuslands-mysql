...
FROM `ejercicio-038-int-jugadores` j
INNER JOIN `ejercicio-038-int-equipos` e ON j.equipo_id = e.id
INNER JOIN `ejercicio-038-int-posiciones` p ON j.posicion_id = p.id;
...
FROM `ejercicio-038-int-equipos` e
LEFT JOIN `ejercicio-038-int-jugadores` j ON e.id = j.equipo_id
GROUP BY e.id;
