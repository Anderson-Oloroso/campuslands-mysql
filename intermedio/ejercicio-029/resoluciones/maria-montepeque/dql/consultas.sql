...
SELECT * FROM vw_productos_disponibles_intermedio
ORDER BY nombre;

-- 2. Productos destacados para la vitrina principal
SELECT * FROM vw_productos_destacados_intermedio
ORDER BY precio DESC;

-- 3. Catalogo publico completo, ordenado por precio
SELECT * FROM vw_catalogo_publico_intermedio
ORDER BY precio DESC;

-- 4. Productos destacados que ademas tienen stock disponible
SELECT d.id_producto, d.nombre, d.categoria, d.precio, d.stock
FROM vw_productos_disponibles_intermedio d
INNER JOIN vw_productos_destacados_intermedio dest ON dest.id_producto = d.id_producto;
...
FROM vw_catalogo_publico_intermedio
GROUP BY categoria
...
--   SELECT stock FROM vw_catalogo_publico_intermedio;
--   -> ERROR 1054 (42S02): Unknown column 'stock' in 'field list'
SELECT * FROM vw_catalogo_publico_intermedio LIMIT 1;
