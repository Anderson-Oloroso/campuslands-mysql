...
FROM `ejercicio-050-int-tatuajes`
WHERE precio > (SELECT AVG(precio) FROM `ejercicio-050-int-tatuajes`);

-- 2. Tatuajes más caros que los de Ana
SELECT cliente, diseno, precio, artista
FROM `ejercicio-050-int-tatuajes`
WHERE precio > (SELECT MAX(precio) FROM `ejercicio-050-int-tatuajes` WHERE artista = 'Ana');
