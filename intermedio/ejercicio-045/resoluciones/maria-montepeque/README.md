# Resolucion - Ejercicio 045 (intermedio)
...
(`usuarios_staging_intermedio`, texto libre, sin restricciones, tal
como llegarian datos crudos de un archivo externo) y la tabla
**final** (`usuarios_intermedio`, tipada y validada). La carga real
sucede con un `INSERT ... SELECT` que limpia los datos de la staging
hacia la final: `TRIM` para espacios, `LOWER` para normalizar la
plataforma, `CAST` para convertir texto a numero, `DISTINCT` para
...
9 filas crudas entraron a la staging; 4 usuarios unicos y validos
llegaron a la tabla final (3 pares de duplicados se colapsaron en 1
cada uno, y 2 filas se descartaron: una con `nombre_usuario` vacio y
otra con `juegos_totales` vacio).
