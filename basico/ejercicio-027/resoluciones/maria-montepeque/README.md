...
## Que hice
Modele la entidad `misiones_basico` para un videojuego de accion y
aventura, pensando en los atributos que realmente distinguen a una
mision del juego: tipo, dificultad, region, recompensas y si ya fue
completada.

## Decisiones tecnicas
- `tipo_mision` y `dificultad` como `ENUM` porque son catalogos
  cerrados y conocidos de antemano, no texto libre.
- `titulo UNIQUE` para no duplicar misiones.
...
3. Top 3 misiones con mejor recompensa de oro.
